...
// Bot protection hands out a clearance cookie on an ordinary page view and
// expects to see it again on the next one. A person picks one up without
// noticing — they load the home page, then search. We arrive cold on a deep
// link with no cookie at all, which is itself part of the fingerprint, and the
// search pages are where that is enforced hardest. So keep what the site sets
// and hand it back, exactly as a browser would.
let cookieJar = "";
let cookieAt = 0;
...
// Load the home page once so the first real request carries a session, the way
// a visitor's would. Best-effort: if it fails we still try the search.
async function warmSession({ force = false } = {}) {
  if (force) {
    cookieJar = "";
    cookieAt = 0;
  } else if (cookieJar && Date.now() - cookieAt < COOKIE_TTL) {
    return;
  }
  await fetchOnce("https://www.autotrader.co.za/");
  // Stamp even when nothing came back, so a site that sets no cookies does not
  // buy an extra home-page request on every single search.
  cookieAt = Date.now();
}

export async function getHtml(url, { warm = false } = {}) {
  const cached = cacheGet(url);
  if (cached) return { status: 200, html: cached, cached: true };

  if (warm) await warmSession();
  ...
  if (!last.html && (last.status === 0 || last.status === 429 || last.status >= 500)) {
    // A refused search is exactly when a stale or missing session is the likely
    // cause, so get a fresh one before asking again rather than repeating the
    // identical request. Forced, or the timestamp above would skip it.
    await warmSession({ force: true });
...
async function searchRows(path, { maxPages = 2, want = 24 } = {}) {
  ...
  for (let p = 1; p <= maxPages; p++) {
    ...
    const res = await getHtml(url, { warm: true });
    status = res.status;
    if (!res.html) {
...
export async function findElectric(price, opts = {}) {
  const { limit = 6 } = opts;
  const EV_PATH = "/cars-for-sale?fueltype=Electric";
  const { rows, reached, status } = await searchRows(EV_PATH);
  ...
  // Returned even when the fetch failed: the page opens perfectly well in the
  // user's own browser, so a link is worth far more than a dead end.
  return {
    list,
    band: banded ? [from, to] : null,
    reached,
    status,
    searchUrl: "https://www.autotrader.co.za" + EV_PATH,
  };
}
