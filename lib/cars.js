...
function browserHeaders(url) {
  return {
    ...
    // Arriving with no referer at all reads as automation.
...
export async function findForSale(make, model, opts = {}) {
  ...
  let { rows, reached } = await searchRows(path);
  ...
  return { list, searchUrl: "https://www.autotrader.co.za" + path, reached };
}
...
export async function findElectric(price, opts = {}) {
  const { limit = 6 } = opts;
  const { rows, reached } = await searchRows("/cars-for-sale?fueltype=Electric");
  ...
  return { list, band: banded ? [from, to] : null, reached };
}
